...
from utils.Getting_valid_directory import (
    GettingValidDirectory,
...
class DeletingSingleFolderClass:
    ...
    def run(self):
        self.logger.info("Welcome to the folder deletion utility!\n")
        ...
        while True:
            ...
            if action == "exit":
                ...
                # getting the folders :
                files = dir_manager.list_files_and_folders(path)
                if not files:
                    print("No files to process. Exiting.")
                    break
                selected_indices = dir_manager.get_valid_files_indices(len(files))
