...
class FolderHandler:
    def __init__(self, enbale_loggin: bool = True):
        if enbale_loggin:
            logging.basicConfig(
                level=logging.INFO,
        ...
        self.logger.info("class deleting folders is good to go\n")

        self.validate_response = ["delete_tree", "exit"]

    def get_user_input(
        self, prompt: str, validate_options: Optional[List[str]]
    ) -> Optional[str]:
        while True:
            response = input(prompt).strip().lower()
            if validate_options is not None and response in validate_options:
                return response
            self.logger.info(
                f"Please enter one of the following options: {', '.join(validate_options or [])}"
            )

    def get_validate_source_directory(self) -> Optional[str]:
        directory_path = Path(
            input(
                "Please, enter the directory where the files or folders are located to process them: "
            )
        )
...
class DeletingNestedFolders:
    def __init__(self, enbale_loggin: bool = True) -> None:
        if enbale_loggin:
            logging.basicConfig(
                level=logging.INFO,
                format="%(asctime)s - %(name)s - %(levelname)s - %(message)s",
        ...
        self.logger = logging.getLogger(__name__)
        self.logger.info("class deleting nested folders is good to go\n")
    ...
    def list_contents_of_selected_folder(self, folder_path: str) -> Optional[list[str]]:
        self.logger.info(f"Contents of folder: {folder_path}\n")
        try:
            items = list(Path(folder_path))
            if not items:
                self.logger.warning("error, the folder is empty\n")
            else:
                for item in items:
                    print(f"-{item.name}")
        except Exception as e:
            self.logger.error(f"error listing the content of the folder : {e}\n")
            raise
...
def main():
    ...
    while True:
        action = dir_manager.get_user_input(
            "Enter 'delete_tree' to remove a folder or 'exit' to quit: ",
        ...
        if action == "exit":
            ...
            if source_dir:
                ...
                if not folders_list:
                    print(
                        "No items available to select for deletion in this directory."
                    )
                else:
                    folders_indices = dir_manager.get_valid_file_indices(
                        len(folders_list)
                    )
                    if folders_indices:  # Only proceed if the user selected items
                        ...
                        consent = (
                            input(
                                "This action is quite destructive. Are you sure you want to proceed? (yes/no): "
                            )
                            .strip()
                            .lower()
                        )
